"""FMP cache service for stock data."""
...
def get_fmp_cached_data(ticker: str) -> Optional[Tuple[pd.DataFrame, datetime]]:
    """
    Get cached FMP stock data for a ticker if it exists and is not expired.
    ...
    with Session(engine) as session:
        statement = select(FMPCache).where(FMPCache.ticker == ticker.upper())
...
def set_fmp_cached_data(ticker: str, data: pd.DataFrame) -> None:
    """
    Cache FMP stock data for a ticker.
...
def purge_expired_fmp_cache() -> int:
    """
    Purge all expired FMP cache entries (older than 24 hours).
    ...
    with Session(engine) as session:
        ...
        statement = select(FMPCache).where(FMPCache.cache_timestamp < cutoff_time)
